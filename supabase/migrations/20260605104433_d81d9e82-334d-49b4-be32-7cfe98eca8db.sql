CREATE TABLE public.match_action_events (
...
GRANT SELECT ON public.match_action_events TO authenticated;
GRANT ALL ON public.match_action_events TO service_role;

ALTER TABLE public.match_action_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own match action events"
ON public.match_action_events
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Admins can view all match action events"
ON public.match_action_events
FOR SELECT
TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

CREATE INDEX idx_match_action_events_user_event_at
ON public.match_action_events (user_id, event_at DESC);

CREATE INDEX idx_match_action_events_suspicious_event_at
ON public.match_action_events (is_suspicious, event_at DESC)
